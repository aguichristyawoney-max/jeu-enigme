...
io.on('connection', (socket) => {
  ...
  socket.on('joueur_rejoindre', ({ code, nom } = {}) => {
    if (!validerString(code, 6) || !validerString(nom, 30)) {
      socket.emit('erreur', 'Données invalides'); return;
    }
    const partie = parties[code];
    if (!partie) {
      socket.emit('erreur', 'Code de partie invalide ou partie expirée !'); return;
    }
    const nomTrim = nom.trim();
    const ancienSocket = Object.values(partie.joueurs).find(j => j.nom === nomTrim);
    if (ancienSocket) delete partie.joueurs[ancienSocket.id];
  ...
  socket.on('admin_bonne_reponse', ({ code, reponse } = {}) => {
    ...
    const partie = parties[code];
    if (!partie || partie.adminId !== socket.id) return;
    io.to(code).emit('bonne_reponse_revelee', { reponse: reponse.trim() });
    toucherPartie(code);
  });
  ...
  socket.on('joueur_quitter_partie', ({ code, nom } = {}) => {
    if (!validerString(code, 6) || !validerString(nom, 30)) {
      socket.emit('quitter_confirme'); return;
    }
    const partie = parties[code];
    if (!partie) {
      socket.emit('quitter_confirme'); return;
    ...
    delete partie.scores[nom];
    delete partie.reponses[nom];
    socket.leave(code);
  ...
  socket.on('disconnect', () => {
    ...
    setTimeout(() => {
      ...
      if (capturedNom) delete partie.scores[capturedNom];
      io.to(capturedCode).emit('joueurs_update', Object.values(partie.joueurs));
      const scores = Object.entries(partie.scores).map(([n, p]) => ({ nom: n, points: p }));
      io.to(capturedCode).emit('points_update', scores);
    }, 5 * 60 * 1000);
  });
});
